...
document.addEventListener('click', function(event) {
  ...
  const container = tab.closest('.motiv-inline-video');
  const comparison = tab.dataset.comparison;

  container.querySelectorAll('.comparison-tab').forEach(button => {
    const isSelected = button === tab;
    button.classList.toggle('is-active', isSelected);
    button.setAttribute('aria-selected', String(isSelected));
  });

  container.querySelectorAll('.comparison-panel').forEach(panel => {
    panel.hidden = panel.dataset.comparisonPanel !== comparison;
  });
});
